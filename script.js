...
function showFlightsTable() {
  ...
  // Get the selected trip type from the dropdown
  const tripType = document.getElementById('dropdownMenuButton1').innerText.trim();
  console.log('Selected Trip Type:', tripType);

  // Get the selected date(s) from Flatpickr
  const dateRangeInput = document.getElementById('dateRange').value.trim();
  let departDate = null;
  let returnDate = null;

  if (tripType === "One way") {
      // One way: Expect a single date
      if (dateRangeInput) {
          departDate = new Date(dateRangeInput).toISOString().split('T')[0];
      }
  } else if (tripType === "Return") {
      // Return: Expect a date range
      const dates = dateRangeInput.split(' to ');
      if (dates.length === 2) {
          departDate = new Date(dates[0]).toISOString().split('T')[0];
          returnDate = new Date(dates[1]).toISOString().split('T')[0];
      }
  }
  ...
  if (returnDate) console.log('Formatted Return Date:', returnDate);

  // Retrieve the selected cabin class from the dropdown button
  let cabinClassText = document.getElementById('dropdownMenuButton2').textContent.trim();
  cabinClassText = cabinClassText.toUpperCase().replace(/\s+/g, '_');

  // Retrieve passengers as an integer
  const passengersValue = document.getElementById('passengersDropdown').value.trim();
  const passengers = parseInt(passengersValue, 10) || 1;
  ...
  const params = new URLSearchParams({
      ...
      ...(returnDate && { returnDate: returnDate }), // Include returnDate only if it exists
      pageNo: 1,
      adults: passengers,
      children: "0,17",
      sort: "FASTEST",
      ...
      .then(data => {
          ...
          if (flightOffers.length > 0) {
              ...
              topTen.forEach(offer => {
                  ...
                  row.innerHTML = `
                      <td>${airlineLogo ? `<img src="${airlineLogo}" alt="Airline Logo" style="width:50px;">` : 'N/A'}</td>
                      <td>${airlineName}</td>
                      <td>${airlineCode}</td>
                      <td>${flightNumber}</td>
